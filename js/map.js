 ...
 */
function convertIntoGeocoding(longitude, latitude){
...
//place where you will be writing your code using my api
	...
	mapInit(mapForm, styles.property, [35.393, 48.282], 7.63);
	const mapDiv = document.getElementById('mapForm');
	// map.on('click', (e) => {
	// 	let coordinates = e.lngLat.wrap();
	// 	renderMarker(coordinates, markerType.medical);
	// });
	// map.addControl(geocoder);
	searchAdress.appendChild(geocoder.onAdd(map));
	// adressInfoBox.addEventListener("cha", function() {
	// 	searching(this.value, defaultSearchSettings)
	// 		.then((resolve) => {

	// 			renderMarker(resolve.features[0].center, markerType.medical);
	// 		});
	// });
		...
		map.loadImage(
            ...
            (error, image) => {
                ...
                map.addLayer({
                    ...
                    'layout': {
                        'icon-image': 'basic', // reference the image
                        'icon-size': 0.1,
						'icon-anchor': 'bottom'
                    }
                });
	...
	map.on('click', (event) => {
		...
		searching(longitude, latitude).then((resolve) => {
			map.getSource('single-point').setData(resolve.features[0].geometry);
